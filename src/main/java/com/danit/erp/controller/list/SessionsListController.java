package com.danit.erp.controller.list;


import com.danit.erp.dto.list.sessions_list.SessionsListRequest;
import com.danit.erp.dto.list.sessions_list.SessionsListResponse;
import com.danit.erp.service.list.SessionsListService;
import java.util.List;
import lombok.AllArgsConstructor;
import org.springframework.web.bind.annotation.CrossOrigin;
import org.springframework.web.bind.annotation.DeleteMapping;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.PutMapping;
import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;

@CrossOrigin("*")
@RestController
@AllArgsConstructor
@RequestMapping("${api.version}/session_list")
public class SessionsListController {
  private final SessionsListService sessionsListService;

  @GetMapping("/")
  public List<SessionsListResponse> getAll() {
    return sessionsListService.findAll();
  }

  @GetMapping("/{id}")
  public SessionsListResponse getById(@PathVariable("id") String userId) throws Exception {
    return sessionsListService.findById(Long.parseLong(userId));
  }

  @DeleteMapping("/{id}")
  public void deleteById(@PathVariable("id") String userId) throws Exception {
    sessionsListService.delete(Long.parseLong(userId));
  }

  @PutMapping("/update")
  public void update(@RequestBody SessionsListRequest sessionsListRequest) {
     sessionsListService.update(sessionsListRequest);
    }



  @PostMapping("/create")
  public SessionsListResponse create(@RequestBody SessionsListRequest sessionsListRequest) {
    return sessionsListService.create(sessionsListRequest);
  }
}
